...
export default function LoginPage() {
  return <Suspense fallback={<main className="min-h-screen bg-[#09090b]" />}><LoginForm /></Suspense>;
}
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (signInError) {
      if (signInError.message.toLowerCase().includes('email not confirmed') || signInError.message.toLowerCase().includes('confirm')) {
        setError('Your account email has not been verified yet. Please check your email/spam folder for a confirmation link, or use Google Sign-In.');
      } else {
  ...
  return (
    <main className="min-h-screen bg-[#09090b] text-[#f4ecdc] flex items-center justify-center px-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-zinc-950/90 p-8 sm:p-10 shadow-2xl backdrop-blur-xl relative z-10">
        ...
        <p className="mt-2 text-sm text-zinc-400">Sign in to open your studio and render projects.</p>
        ...
        >
          ...
          <span className="text-[10px] text-emerald-400 font-mono font-normal ml-1 bg-emerald-500/10 px-1.5 py-0.5 rounded border border-emerald-500/20">Recommended</span>
        </button>
        ...
        <p className="mt-8 text-center text-sm text-zinc-400">
          New here?{' '}
          <Link className="text-violet-400 font-bold hover:underline" href={`/auth/sign-up?next=${encodeURIComponent(nextPath)}`}>
