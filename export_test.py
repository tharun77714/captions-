...
"""
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        for s_idx, seg in enumerate(segments):
            ...
            if has_bg and layout_words:
                lines = []
                current_line = []
                current_y = None
                
                # Group words into lines by Y coordinate
                for w in layout_words:
                    # Allow 10px threshold for Y coordinate variations
                ...
                for line in lines:
                    ...
                    max_x = max(w['x'] + w['w'] for w in line)
                    min_y = min(w['y'] for w in line)
                    max_y = max(w['y'] + w['h'] for w in line)
                    
                    # Apply CSS padding manually (already converted to native coordinates via scale in frontend? No, wait, css_padding is in css px)
                    # Let's scale css_padding here, because padding is sent from style or hardcoded.
                    # Wait, css_padding_x and y are still defined in export.py. I can just use them scaled.
                    pad_x = int(round(css_padding_x * scale))
                    pad_y = int(round(css_padding_y * scale))

                    bx = int(round(min_x - pad_x))
                    by = int(round(min_y - pad_y))
                    bw = int(round(max_x - min_x + pad_x * 2))
                    bh = int(round(max_y - min_y + pad_y * 2))
...
def render_video(project_id: str, s3_key: str, measurements: dict = None, subtitle_mode: str = "original"):
    import boto3
...
def trigger_export(data: dict):
    ...
    render_video.spawn(project_id, s3_key, measurements, subtitle_mode)
